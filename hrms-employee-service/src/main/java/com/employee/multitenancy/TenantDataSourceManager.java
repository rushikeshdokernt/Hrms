...
public class TenantDataSourceManager {
    ...
    /**
     * Retrieves an existing HikariDataSource or creates and warms up a new one.
     */
    public synchronized HikariDataSource getOrCreateDataSource(TenantConnectionConfigDto tenantConfig) {
        ...
        log.info("Creating new HikariCP connection pool for tenant [{}]...", tenantCode);
        ...
        config.setPoolName("HikariPool-Employee-Tenant-" + tenantCode);
        ...
        // 3. Warmup: eagerly acquire a connection to initialize the pool
        try (Connection connection = dataSource.getConnection()) {
            log.info("Connection pool successfully initialized for tenant [{}]. Valid: {}",
                    tenantCode, connection.isValid(2));
        } catch (Exception ex) {
            log.error("Failed to validate connection pool for tenant [{}]: {}", tenantCode, ex.getMessage());
            dataSource.close();
            throw new RuntimeException("Database connection failure for tenant: " + tenantCode, ex);
        }

        // 4. Register in map & routing datasource under tenantCode, subdomain, and tenantId
