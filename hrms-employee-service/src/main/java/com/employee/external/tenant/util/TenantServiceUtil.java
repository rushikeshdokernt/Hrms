package com.employee.external.tenant.util;
...
import com.employee.external.tenant.client.TenantFeignClient;
...
public class TenantServiceUtil {
    ...
    // In-memory cache to prevent repeated Feign calls — TTL 10 minutes
    ...
    public TenantConnectionConfigDto resolveTenantByDomain(String domain) {
        ...
        try {
            ResponseEntity<TenantConnectionConfigDto> response =
                    tenantFeignClient.resolveTenantByDomain(domain.trim());
    ...
    public TenantConnectionConfigDto getTenantByCode(String tenantCode) {
        ...
        try {
            ResponseEntity<TenantConnectionConfigDto> response =
                    tenantFeignClient.getTenantConfigByCode(tenantCode.trim());
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                TenantConnectionConfigDto config = response.getBody();
                cacheTenant(config);
                return config;
    ...
    public TenantConnectionConfigDto getTenantBySubdomain(String subdomain) {
        ...
        try {
            ResponseEntity<TenantConnectionConfigDto> response =
                    tenantFeignClient.getTenantConfigBySubdomain(subdomain.trim());
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                TenantConnectionConfigDto config = response.getBody();
                cacheTenant(config);
                return config;
    ...
    public TenantConnectionConfigDto getTenantById(String tenantId) {
        ...
        try {
            ...
            ResponseEntity<TenantConnectionConfigDto> response =
                    tenantFeignClient.getTenantConfigById(uuid);
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                TenantConnectionConfigDto config = response.getBody();
                cacheTenant(config);
                return config;
